...
class Optparser
  ...
  def self.parse(args)
    ...
    opts = OptionParser.new do |opts|
      ...
      opts.banner = "Usage: f5_gtm_pool_set_fallback_lb_method.rb [options]"
      opts.separator ""
...
def gtm_set_fallback_lb_method(lb, pool_names, lb_methods)
  lb.icontrol.globallb.gtm_pool.set_fallback_lb_method( pool_names, lb_methods)
end
...
REQ_PARAMS = [:bigip, :pool_name, :port, :bigip_conn_conf]
...
gtm_set_fallback_lb_method(lb, pool_names, lb_methods)

# ruby -W0 f5_gtm_set_fallback_lb_method.rb --bigip_conn_conf "..\private-fixtures\config-andy-qa-gtm-ve.yml" --pool_name dr.enduser.myfake.vip.tpgtm.info_80 -b 192.168.106.x
