...
if service_list.empty?
  ...
  output = %x{ruby -W0 f5_pool_create.rb --name #{vs_yaml_conf.pool["name"]} #{member_list} --bigip 192.168.106.13}
  ...
  service_list.each do |service_num|
    ...
    output = %x{ruby -W0 f5_pool_create.rb --name #{current_service_conf.pool["name"]} #{member_list} --bigip 192.168.106.13}
